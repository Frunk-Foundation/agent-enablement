...
class EnablerMcp:
        ...
        self._register(
            ToolDef(
                ...
                input_schema={
                    "type": "object",
                    "properties": {
                        "action": {"type": "string", "enum": ["file", "folder", "help"]},
                        "args": {"type": "object"},
                        "async": {"type": "boolean"},
                    },
                    "required": ["action"],
    ...
    def _help_example_args(*, tool_name: str, action: str) -> dict[str, Any]:
        examples: dict[tuple[str, str], dict[str, Any]] = {
            ("credentials.exec", "help"): {"action": "ensure"},
            ...
            ("taskboard.exec", "unclaim"): {"taskId": "task-123"},
            ("taskboard.exec", "done"): {"taskId": "task-123", "result": "completed"},
            ("taskboard.exec", "fail"): {"taskId": "task-123", "result": "blocked by external dependency"},
    ...
    def _dispatch_share(self, action: str, args: dict[str, Any], *, g: GlobalOpts) -> Any:
        if action == "help":
            target_action = str(args.get("action") or "").strip()
            return {
                "kind": "enabler.mcp.help.v1",
    ...
    def _tool_share_exec(self, args: dict[str, Any]) -> Any:
        g = self._g_for_agent(self._require_bound_agent_id())
        action = str(args.get("action") or "").strip()
        action_args = args.get("args")
        if not isinstance(action_args, dict):
