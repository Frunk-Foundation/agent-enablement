...
from .cli_shared import ENABLER_BUNDLE_ENDPOINT
...
class _InsertionOrderTyperGroup(typer.core.TyperGroup):
    def list_commands(self, ctx: click.Context) -> list[str]:
        ...
        tail = [n for n in ("pack-build", "pack-publish") if n in names]
        head = [n for n in names if n not in set(lead + tail)]
...
def _http_get_to_file(*, url: str, out_path: Path, timeout_seconds: int = 120) -> int:
    ...
    try:
        ...
        data = e.read() if hasattr(e, "read") else b""
        raise OpError(
            f"http download failed: status={int(getattr(e, 'code', 0) or 0)} body={data.decode('utf-8', errors='replace')}"
        ) from e
    except URLError as e:
        raise OpError(f"http download failed: {e}") from e
...
def _default_bundle_output_path(g: GlobalOpts) -> Path:
    name = f"agent-enablement-bundle-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.zip"
    return (_artifact_root(g) / "bundles" / name).resolve()


def _connection_cache_file(g: GlobalOpts) -> Path:
    return (_artifact_root(g) / "connection.json").resolve()


def _load_connection_doc(g: GlobalOpts) -> dict[str, Any] | None:
    path = _connection_cache_file(g)
    if not path.exists():
        return None
    try:
        return _load_json_object(
            raw=path.read_text(encoding="utf-8"),
            label=f"connection JSON at {path}",
        )
    except UsageError:
        return None


def _write_connection_cache_from_doc(*, g: GlobalOpts, doc: dict[str, Any]) -> Path:
    path = _connection_cache_file(g)
    _write_secure_json(path=path, obj=doc)
    return path


def _taskboard_endpoint_from_connection_doc(doc: dict[str, Any]) -> str:
    taskboard = doc.get("taskboard")
    if not isinstance(taskboard, dict):
        return ""
    for key in ("invokeUrl", "endpoint", "url"):
        val = str(taskboard.get(key) or "").strip()
        if val:
            return val.rstrip("/")
    return ""


def _taskboard_endpoint_from_connection_cache(g: GlobalOpts) -> str:
    doc = _load_connection_doc(g)
    if not isinstance(doc, dict):
        return ""
    return _taskboard_endpoint_from_connection_doc(doc)


def _shortlinks_create_url_from_connection_doc(doc: dict[str, Any]) -> str:
    shortlinks = doc.get("shortlinks")
    if not isinstance(shortlinks, dict):
        return ""
    return str(shortlinks.get("createUrl") or "").strip()


def _shortlinks_redirect_base_url_from_connection_doc(doc: dict[str, Any]) -> str:
    shortlinks = doc.get("shortlinks")
    if not isinstance(shortlinks, dict):
        return ""
...
def _shortlinks_create_url_from_connection_cache(g: GlobalOpts) -> str:
    doc = _load_connection_doc(g)
    if not isinstance(doc, dict):
        return ""
    return _shortlinks_create_url_from_connection_doc(doc)


def _shortlinks_redirect_base_url_from_connection_cache(g: GlobalOpts) -> str:
    doc = _load_connection_doc(g)
    if not isinstance(doc, dict):
        return ""
    return _shortlinks_redirect_base_url_from_connection_doc(doc)


def _files_public_base_url_from_connection_doc(doc: dict[str, Any]) -> str:
    files = doc.get("files")
...
def _files_public_base_url_from_connection_cache(g: GlobalOpts) -> str:
    doc = _load_connection_doc(g)
    if not isinstance(doc, dict):
        return ""
    return _files_public_base_url_from_connection_doc(doc)


def _connection_region_from_doc(doc: dict[str, Any]) -> str:
    return str(doc.get("awsRegion") or "").strip()


def _connection_region_from_cache(g: GlobalOpts) -> str:
    doc = _load_connection_doc(g)
    if not isinstance(doc, dict):
        return ""
    return _connection_region_from_doc(doc)
...
def _credentials_location_manifest(
    *,
    ...
    connection_path = (root / "connection.json").resolve()
    ...
    return {
        ...
        "paths": {
            ...
            "connectionJson": str(connection_path),
        ...
        "exists": {
            ...
            "connectionJson": connection_path.exists(),
        ...
        "contains": {
            ...
            "connectionJson": "Bundle/runtime endpoint metadata from `enabler bundle`.",
...
def _print_credentials_location_manifest_human(manifest: dict[str, Any]) -> None:
    ...
    sys.stdout.write(f"- connection.json: {paths.get('connectionJson', '')} (exists={exists.get('connectionJson', False)})\n")
    sys.stdout.write(f"  contains: Bundle connection metadata and runtime endpoints.\n")
...
def _resolve_runtime_credentials_doc(args: argparse.Namespace, g: GlobalOpts) -> dict[str, Any]:
    ...
    if not g.auto_refresh_creds:
        ...
        raise UsageError("missing credentials cache (run 'enabler credentials' or omit --no-auto-refresh-creds)")
...
def _build_help_credentials_banner(*, role: str | None = None, ctx: click.Context | None = None) -> str:
    ...
    explicit_bucket = _help_param_from_context(ctx, "bucket")
    if " pack-" in f" {path} ":
        if explicit_bucket:
            lines.append(f"Bucket: {explicit_bucket} (source=--bucket)")
        else:
            bucket = _help_admin_stack_output_value(stack=stack_name, key="CommsSharedBucketName")
            if bucket:
                lines.append("Bucket: " + bucket + " (source=stack output CommsSharedBucketName)")
            else:
                lines.append("Bucket: unknown (source=stack output CommsSharedBucketName)")
...
def cmd_agent_bundle(args: argparse.Namespace, g: GlobalOpts) -> int:
    try:
        basic = auth_inputs.resolve_basic_credentials(
            username=args.username,
            password=args.password,
        ...
        resolved = auth_inputs.resolve_agent_request_auth_client(
            endpoint=args.endpoint,
            endpoint_env_names=(ENABLER_BUNDLE_ENDPOINT,),
            api_key=args.api_key,
            api_key_env_names=(ENABLER_API_KEY,),
            env_or_none=_env_or_none,
            missing_endpoint_error=(
                f"missing bundle endpoint (pass --endpoint or set {ENABLER_BUNDLE_ENDPOINT})"
            ),
            missing_api_key_error=f"missing api key (pass --api-key or set {ENABLER_API_KEY})",
        )
    except auth_inputs.AuthInputError as e:
    ...
    url = resolved.endpoint
    ttl = str(args.ttl_seconds or "").strip()
    if ttl:
        url = f"{resolved.endpoint}?ttlSeconds={ttl}"
    ...
    body_text = data.decode("utf-8", errors="replace")
    if status < 200 or status >= 300:
        raise OpError(f"bundle request failed: status={status} body={body_text}")

    try:
        parsed = json.loads(body_text)
    except Exception as e:
        raise OpError(f"bundle response is not JSON: {e}") from e

    connection = parsed.get("connection")
    if isinstance(connection, dict):
        _write_connection_cache_from_doc(g=g, doc=connection)

    no_download = bool(args.no_download)
    print_response = bool(getattr(args, "print_response", False))

    if no_download:
        if args.out:
            _eprint("warning: --out ignored when --no-download is set")
        if print_response:
            _print_json(parsed, pretty=g.pretty)
        return 0

    bundle_url = str(parsed.get("bundleUrl") or "").strip()
    if not bundle_url:
        raise OpError("bundle response missing bundleUrl")

    out_path = Path(args.out).expanduser().resolve() if args.out else _default_bundle_output_path(g)

    _http_get_to_file(url=bundle_url, out_path=out_path)
    if print_response:
        _print_json(parsed, pretty=g.pretty)
...
def _resolve_share_runtime(args: argparse.Namespace, g: GlobalOpts) -> tuple[Any, str, str, str]:
    ...
    region = _connection_region_from_cache(g)
    if not region:
        raise UsageError("missing awsRegion in bundle connection.json (run 'enabler bundle' first)")
    public_base_url = _files_public_base_url_from_connection_cache(g)
...
def cmd_shortlinks_create(args: argparse.Namespace, g: GlobalOpts) -> int:
    endpoint = _shortlinks_create_url_from_connection_cache(g)
    if not endpoint:
        raise UsageError("missing shortlinks create endpoint in bundle connection.json (run 'enabler bundle' first)")
    redirect_base_url = _shortlinks_redirect_base_url_from_connection_cache(g)
    if not redirect_base_url:
        raise UsageError("missing shortlinks redirect base url in bundle connection.json (run 'enabler bundle' first)")
    root_doc = _resolve_runtime_credentials_doc(args, g)
    id_token = _doc_find_id_token(root_doc)
    if not id_token:
        raise UsageError("cached credentials missing cognitoTokens.idToken (run 'enabler credentials')")
    try:
...
def cmd_shortlinks_resolve_url(args: argparse.Namespace, g: GlobalOpts) -> int:
    base_url = _shortlinks_redirect_base_url_from_connection_cache(g)
    if not base_url:
        raise UsageError("missing shortlinks redirect base url in bundle connection.json (run 'enabler bundle' first)")
...
def _taskboard_endpoint_for_args(args: argparse.Namespace, g: GlobalOpts) -> str:
    del args
    endpoint = _taskboard_endpoint_from_connection_cache(g)
    if not endpoint:
        raise UsageError("missing taskboard endpoint in bundle connection.json (run 'enabler bundle' first)")
    return endpoint
...
def cmd_pack_build(args: argparse.Namespace, g: GlobalOpts) -> int:
    from enablement_pack.build_pack import build_pack

    repo_root = Path(__file__).resolve().parents[1]
    manifest = Path(args.manifest or (repo_root / "enablement_pack" / "manifest.yaml")).resolve()
    output_root = Path(args.output_root or (repo_root / "enablement_pack" / "dist")).resolve()
    base_url = str(args.base_url or "https://example.invalid/agent-enablement/latest")
    res = build_pack(manifest_path=manifest, output_root=output_root, base_url=base_url)
    _print_json(res, pretty=g.pretty)
    return 0


def cmd_pack_publish(args: argparse.Namespace, g: GlobalOpts) -> int:
    from enablement_pack.publish_pack import publish

    # publish_pack follows repo convention and reads AWS_PROFILE/AWS_REGION from env.
    _aws_profile_region_from_env()
    bucket = str(getattr(args, "bucket", "") or "").strip()
    if not bucket:
        session = _account_session()
        bucket = _require_stack_output(session, stack=g.stack, key="CommsSharedBucketName")
    repo_root = Path(__file__).resolve().parents[1]
    dist_root = Path(args.dist_root or (repo_root / "enablement_pack" / "dist")).resolve()
    res = publish(
        bucket=bucket,
...
def agent_handoff_create(
    ...
    bundle_endpoint: str | None = typer.Option(
        None,
        "--bundle-endpoint",
        help="Override bundle endpoint (otherwise stack output BundleInvokeUrl)",
    ),
...
@app.command(
    "bundle",
    help="Download the enablement ZIP, persist bundle connection.json metadata, and optionally print bundle response JSON.",
)
def agent_bundle(
    ctx: typer.Context,
    username: str | None = typer.Option(None, "--username", help=f"Cognito username (or env {ENABLER_COGNITO_USERNAME})"),
    password: str | None = typer.Option(None, "--password", help=f"Cognito password (or env {ENABLER_COGNITO_PASSWORD})"),
    endpoint: str | None = typer.Option(None, "--endpoint", help=f"Override bundle endpoint (or env {ENABLER_BUNDLE_ENDPOINT})"),
    api_key: str | None = typer.Option(None, "--api-key", help=f"Override API key value (or env {ENABLER_API_KEY})"),
    ttl_seconds: str | None = typer.Option(None, "--ttl-seconds", help="Presign TTL seconds (default: 900; max: 3600)"),
    out: str | None = typer.Option(
        None,
        "--out",
        help="Write downloaded zip to this path (default: <creds-cache-dir>/bundles/agent-enablement-bundle-<timestamp>.zip; ignored with --no-download)",
    ),
    no_download: bool = typer.Option(False, "--no-download", help="Skip ZIP download (prints nothing unless --print-response)"),
    print_response: bool = typer.Option(False, "--print-response", help="Print bundle response JSON"),
) -> None:
    _invoke(
        ctx,
        cmd_agent_bundle,
...
@admin_app.command("pack-build", help="Build the link-first enablement pack locally.")
def pack_build(
    ctx: typer.Context,
    manifest: str | None = typer.Option(None, "--manifest", help="Manifest path (default: enablement_pack/manifest.yaml)"),
    output_root: str | None = typer.Option(None, "--output-root", help="Output root (default: enablement_pack/dist)"),
    base_url: str | None = typer.Option(None, "--base-url", help="Base URL used to render link-first URLs"),
) -> None:
    _invoke(ctx, cmd_pack_build, manifest=manifest, output_root=output_root, base_url=base_url)


@admin_app.command("pack-publish", help="Publish a built enablement pack to S3.")
def pack_publish(
    ctx: typer.Context,
    bucket: str | None = typer.Option(
        None,
        "--bucket",
        help="Target S3 bucket (default: stack output CommsSharedBucketName)",
    ),
    version: str = typer.Option(..., "--version"),
    dist_root: str | None = typer.Option(None, "--dist-root", help="Dist root (default: enablement_pack/dist)"),
    prefix: str = typer.Option("agent-enablement", "--prefix"),
